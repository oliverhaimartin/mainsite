...
RETRY_BUTTON_CLASS = "retry-now", _createRetryButton = function() {
		...
		return Submit.prototype.inReview = !1, Submit.prototype.lockedFailedSubmitStatus = !0, Submit.prototype.defaults = {
				...
				var list;
				return list = this.generateList(), this.inReview = !1, list.length ? (this.showMessage(App.Texts.review.replace(/{{NUM}}/g, list.length)), this.$footer.addClass("review")) : (!1 !== this.canSubmitForm && this.sendForm(), this.canSubmitForm = !1)
				...
				if (self = this, list = this.generateList(this.controls, this.numQuestions), id = list[0], "default" === this.getBrowser() ? $("body").removeClass("footer-error footer-submit") : $("body").removeClass("full-footer"), this.$footer.removeClass("review"), void 0 !== id) return this.inReview = !0, this.disableUseFocus(), $(list).each(function(index, val) {
			...
			Submit.prototype.setReview = function(r) {
				...
				null == retry && (retry = !1), self = this, data = {
					...
					return self.error(App.Texts[errCode]), self.canSubmitForm = !0, self.onSubmit()
			...
			return PRELOAD_IMAGES_MAX_DEEP = 3, Logic.prototype.controls = [], Logic.prototype.logicJumpsController = [], Logic.prototype.setPaymentField = function() {
				...
				null == delay && (delay = !1), null == check && (check = !0), delay && (delay = this.scroll.settings["fast" === delay ? "delayAutoScrollFast" : "delayAutoScroll"]), current = this.scroll.getCurrent(), this.scroll.getCurrentControl().setValue(), displayedControls = _.filter(this.controls, function(control) {
					...
					return typeform.Form.footer.inReview ? (afterControls = _.rest(displayedControls, current), _.every(afterControls, function(control, index) {
						var valid;
						return valid = control.validate(!1, !0), _.delay(function() {
							if (!valid) return _this.scroll.scrollTo(current + index)
						}, delay), valid
				...
				if (isPassed && !isAlreadyTracked) {
					...
					this.trackedQuestions[control.id] = !0, App.Utils.UserAnalytics.sendQuestionPassedEvent(formField.label)
				}
...
ControlsFactory = function() {
	function ControlsFactory(fieldsData, browser, currencyFormatter, priceCalculator, featureDetection, settings, numberFormatter) {
		this.fieldsData = fieldsData, this.browser = browser, this.currencyFormatter = currencyFormatter, this.priceCalculator = priceCalculator, this.featureDetection = featureDetection, this.settings = settings, this.numberFormatter = numberFormatter
	}
	return ControlsFactory.prototype.build = function() {
		...
		}, $controls.filter(".statement").statement(commonOptions), $controls.filter(".textfield").not("li[id*='creditCard']").textfield(commonOptions), $controls.filter(".textarea").textarea(commonOptions), $controls.filter(".number").not("li[id*='creditCard']").textfield(commonOptions), $controls.filter(".list").list(commonOptions), $controls.filter(".list-image").list(commonOptions), $controls.filter(".yes-no").yesNo(commonOptions), $controls.filter(".terms").terms(commonOptions), $controls.filter(".email").textfield(commonOptions), $controls.filter(".website").textfield(commonOptions), $controls.filter("li[id$='creditCardNumber']").textfield(_.extend({}, commonOptions, {
		...
		return $controls.filter(".statement").statement(), $controls.filter(".textfield").not("li[id*='creditCard']").textfield(), $controls.filter(".textarea").textarea(), $controls.filter(".number").textfield(), $controls.filter(".list").list({
			confirmButton: confirmButton
		}), $controls.filter(".list-image").list({
			confirmButton: confirmButton
		}), $controls.filter(".yes-no").yesNo(), $controls.filter(".terms").terms(), $controls.filter(".email").textfield(), $controls.filter(".website").textfield(), $controls.filter("li[id$='creditCardNumber']").textfield({
			validator: App.validation.creditCardNumber
...
OpinionScaleView = function(_super) {
	...
	return __extends(OpinionScaleView, _super), OpinionScaleView.prototype.choicesView = null, OpinionScaleView.prototype.fieldModel = null, OpinionScaleView.prototype.template = "", OpinionScaleView.prototype.$choices = null, OpinionScaleView.prototype.$labels = null, OpinionScaleView.prototype.getFieldModel = function() {
		...
		return null == this[_name = "$" + label] && (this[_name] = this.$el.find("." + label)),
			this["$" + label].html(data[label]), this
	}, OpinionScaleView
...
ChoicesView = function(_super) {
	...
	return __extends(ChoicesView, _super), ChoicesView.prototype.tagName = "ul", ChoicesView.prototype.collection = null, ChoicesView.prototype.className = "choices", ChoicesView.prototype.$document = null, ChoicesView.prototype.createChoiceView = null, ChoicesView.prototype.keyboard = null, ChoicesView.prototype.options = null, ChoicesView.prototype.defaults = {
			...
			return null != this.keyboard && this.$document.on("keyup.choices", this.onKeyup), this.$el.on("click.choices", ".choice", this.onClickChoice), this.$el.on("mouseover.choices", ".choice", this.onMouseover), this.$el.on("mouseout.choices", ".choice", this.onMouseleave), this
...
LogicJumpsController = function() {
	...
	return LogicJumpsController.prototype.jumps = null, LogicJumpsController.prototype.controls = null, LogicJumpsController.prototype.values = [], LogicJumpsController.prototype.onBackwardJump = function() {}, LogicJumpsController.prototype.setControls = function(controls) {
			...
			return jumpTo = this.getDestinationIndex(destination), control = this.getControlById(controlId), jumpFrom = _.indexOf(this.controls, control), jumpTo < jumpFrom ? (this.doBackwardJump(control, destination), jumpTo = _.size(this.controls)) : this.checkFromControlIndex(jumpTo), this.hideControls(jumpFrom + 1, jumpTo), this.activeOutro(destination), !1
...
StripeService = function() {
		function StripeService(Stripe, key) {
			this.Stripe = Stripe, this.Stripe.setPublishableKey(key)
		}
...
var Textfield = function(elem, options) {
	...
	this.value = "", this.pipeValue = "", this.isCompleted = !1, this.required = elem.hasClass("required"), this.unique = elem.hasClass("unique"), this.done = !1, this.required, isEmailType = elem.hasClass("email"), isWebsiteType = elem.hasClass("website"), isNumberType = elem.hasClass("number"), this.confirmText = "default", this.attachLimit = {
		...
		$span = elem.find(".question > span"), $item = elem.find(".item").children("span"),
			$input = elem.find("input"), "" === $span.html() && elem.hide(), $confirmButton = elem.find(".container.confirm"), isWebsiteType && "" === $input.val() && $input.val("http://"), isNumberType && (min = parseInt($input.attr("min")), max = parseInt($input.attr("max")))
...
var List = function(elem, options) {
	...
	this.value = {
			choices: undefined,
			other: ""
		}, this.logicValue = {}, this.pipeValue = "", this.isCompleted = !1, this.isCreditCard = !!elem.attr("id") && elem.attr("id").indexOf("creditCard") > -1, this.required = elem.hasClass("required"), this.multiple = elem.hasClass("multiple"), randomize = elem.hasClass("randomize"), supersize = elem.hasClass("supersize"),
		this.vertical = elem.hasClass("vertical"), this.done = !1, multiple = this.multiple, this.required, vertical = this.vertical, isPictureChoice = elem.hasClass("list-image"), this.confirmText = "default", this.attachLimit = multiple ? {
			"default": 165,
			large: 165,
			small: 124
		} : {
...
Group = function() {
	...
	return Group.prototype.$elem = null, Group.prototype.$span = null, Group.prototype.$item = null, Group.prototype.$button_wrap = null, Group.prototype.value = "", Group.prototype.pipeValue = "", Group.prototype.isCompleted = !0, Group.prototype.required = !1, Group.prototype.done = !1, Group.prototype.confirmText = "default", Group.prototype.attachLimit = {
		...
		return this.isPaymentField() && this.renderStripeDescription(), this
		...
		return _.some(this.getValues(), function(value, key) {
		...
		this[attr] = value, "isActive" === attr || "isVisible" === attr ? this.isVisible && this.isActive ? this.$elem.addClass("active") : this.$elem.removeClass("active") : "index" === attr && value && this.$item.html(value)
		...
		return callback && callback(), !0
	}, Group.prototype.resize = function() {}, Group.prototype.update = function(obj) {
...
UrlModelFactory = App.UrlModelFactory, AppFactory = function() {
		...
		return AppFactory.prototype.$typeform = null, AppFactory.prototype.observer = null, AppFactory.prototype.browser = null, AppFactory.prototype.build = function() {
			...
			}, updateSubmitButtonText(), priceCalculator.on("change:total", updateSubmitButtonText), null != paymentField && paymentField.model.on("change:filled", updateSubmitButtonText), "field" === settings.parent && (superframe.initUpdateEvent && superframe.initUpdateEvent(function(evt, obj) {
